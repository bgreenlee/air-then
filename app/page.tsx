...
export default function Home() {
  ...
  return <main>
    ...
    <section className="explorer" id="explore">
      ...
      <div className="history-card"><div><p className="eyebrow">Historical view</p><h3>Every available year, at a glance.</h3><p>Each strip condenses a full year into daily AQI. Select a year to inspect it above.</p></div><div className="history-timeline"><div className="history-months" aria-hidden="true">{months.map(month => <span key={month}>{month}</span>)}</div><div className="year-strips">{historicalYears.map(y => { const row = demoDays(y); return <button className={y === year ? "year-strip selected" : "year-strip"} onClick={() => setYear(y)} key={y} aria-label={`Show AQI calendar for ${y}`}><b>{y}</b><span className="year-ramp" style={{ backgroundImage: dailyGradient(row) }}/><em>{y === year ? "Viewing" : ""}</em></button>; })}</div></div></div>
