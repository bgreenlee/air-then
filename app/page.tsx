...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero" id="top">
      ...
      </form>{searchFocused && suggestions.length > 0 && <div className="autocomplete" role="listbox">{suggestions.map((result) => <button key={`${result.area_id}-${result.label ?? result.name}`} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { selectLocation(result); setSearchFocused(false); setIsCurrentLocation(false); }}><span>{result.label ?? result.name}</span><small>{result.type === "city" ? "City" : "Metro area"}</small></button>)}</div>}</div>
    ...
    <section className="explorer" id="explore">
      <div className="result-head"><div><p className="eyebrow">Search result</p><h2>{displayArea(dataArea)}</h2><p className="subtle">Metro area · {dataSource}</p><p className="resolution-note"><b>Why this source?</b> {isCurrentLocation ? "Your current location resolves to the nearest EPA metro area." : "This result has strong daily monitoring coverage."}</p></div><div className="year-controls"><button onClick={() => setYear(year - 1)} aria-label="Previous year"><ChevronLeft size={20} strokeWidth={1.8} aria-hidden="true"/></button><strong>{year}</strong><button onClick={() => setYear(year + 1)} disabled={year >= currentYear} aria-label="Next year"><ChevronRight size={20} strokeWidth={1.8} aria-hidden="true"/></button></div></div>
    ...
    <footer><span>AirThen is built from <a href="https://www.epa.gov/outdoor-air-quality-data" target="_blank" rel="noreferrer">EPA AirData</a> bulk files and Census geographic references.</span><span className="footer-links"><a className="github-link" href="https://github.com/bgreenlee/clear-skies" target="_blank" rel="noreferrer" aria-label="AirThen on GitHub"><img src="https://github.githubassets.com/favicons/favicon.svg" alt=""/></a></span></footer>
