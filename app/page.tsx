...
export default function Home() {
  ...
  return <main>
    <nav><div className="brand-block"><a className="brand" href="#top"><img className="brand-mark" width="28" height="28" src="/airthen-mark.svg" alt=""/>AirThen</a><span className="brand-subtitle">Historical air quality</span></div><div className="nav-links"><a href="https://www.epa.gov/outdoor-air-quality-data" target="_blank" rel="noreferrer">EPA AirData <ExternalLink size={13} strokeWidth={1.8} aria-hidden="true"/></a></div></nav>
    ...
    <footer><span>AirThen is built from <a href="https://www.epa.gov/outdoor-air-quality-data" target="_blank" rel="noreferrer">EPA AirData</a> bulk files and Census geographic references.</span><span className="footer-links"><a className="github-link" href="https://github.com/bgreenlee/air-then" target="_blank" rel="noreferrer" aria-label="AirThen on GitHub"><img src="https://github.githubassets.com/favicons/favicon.svg" alt=""/></a></span></footer>
...
function InternationalView({ measurements, stats }: { measurements: Measurement[]; stats: { pollutant: string; units: string; median: number; max: number; days: number }[] }) {
  ...
  const pollutantMax = new Map(stats.map((stat) => [stat.pollutant, stat.max]));
  ...
  const yearStrip = (stripYear: number) => {
    ...
    const values = days.map((date) => Math.max(...(byDate.get(date) ?? []).map((row) => row.value / Math.max(1, pollutantMax.get(row.pollutant) ?? row.value)), 0));
    return days.length ? `linear-gradient(90deg, ${values.map((value, index) => { const color = `hsl(${Math.max(8, 145 - value * 137)} 62% ${Math.max(34, 82 - value * 38)}%)`; const start = index / values.length * 100; const end = (index + 1) / values.length * 100; return `${color} ${start}%, ${color} ${end}%`; }).join(", ")})` : undefined;
  };
  const internationalTrends = years.map((trendYear) => {
    const values = [...new Set(measurements.filter((row) => row.date.startsWith(`${trendYear}-`)).map((row) => row.date))].map((date) => Math.max(...(byDate.get(date) ?? []).map((row) => row.value / Math.max(1, pollutantMax.get(row.pollutant) ?? row.value)), 0)).sort((a, b) => a - b);
  ...
  const internationalScale = Math.max(.5, ...internationalTrends.flatMap((trend) => trend.p90 === null ? [] : [trend.p90]));
  ...
  return <>
    ...
    <div className="chart-card"><div className="chart-heading"><div><p className="eyebrow">Combined daily pollutant calendar</p><p>Each square combines the relative levels of all reported pollutants. Hover a day for the individual readings.</p></div></div><div className="calendar-wrap"><div className="month-labels">{months.map(m => <span key={m}>{m}</span>)}</div><div className="calendar" aria-label={`Combined daily pollutant calendar for ${selectedYear}`}>{calendar.map((date, index) => { const rows = date ? byDate.get(date) ?? [] : []; const burden = rows.length ? Math.max(...rows.map((row) => row.value / Math.max(1, pollutantMax.get(row.pollutant) ?? row.value))) : 0; const color = rows.length ? `hsl(${Math.max(8, 145 - burden * 137)} 62% ${Math.max(34, 82 - burden * 38)}%)` : undefined; return <div key={index} role={date ? "img" : undefined} tabIndex={rows.length ? 0 : undefined} aria-label={rows.length ? `${date}: ${rows.map((row) => `${row.pollutant} ${row.value.toFixed(1)} ${row.units}`).join(", ")}` : undefined} onMouseMove={(event) => date && rows.length && setHovered({ date, x: event.clientX, y: event.clientY })} onMouseLeave={() => setHovered(null)} onFocus={(event) => { if (date && rows.length) { const box = event.currentTarget.getBoundingClientRect(); setHovered({ date, x: box.left + box.width / 2, y: box.bottom }); } }} onBlur={() => setHovered(null)} className={`day ${date ? "" : "empty"}`} style={color ? { backgroundColor: color } : undefined}/>; })}</div></div><div className="legend"><span>Lower relative levels</span><i className="good"/><i className="moderate"/><i className="sensitive"/><i className="unhealthy"/><span>Higher relative levels</span><i className="missing"/><span>No data</span></div></div>
    ...
    <div className="trend-card"><div className="trend-heading"><div><p className="eyebrow">Annual trend</p><p>Median and P90 show relative combined pollutant levels; bars count days above the midpoint.</p></div><div className="trend-legend"><span><i className="median-dot"/>Median</span><span><i className="p90-dot"/>P90</span></div></div><div className="trend-plot" aria-label="Annual combined pollutant trend">{internationalTrends.map((trend) => { const medianTop = trend.median === null ? null : 100 - trend.median / internationalScale * 100; const p90Top = trend.p90 === null ? null : 100 - trend.p90 / internationalScale * 100; return <div className="trend-column" key={trend.year}><div className="trend-range-area">{medianTop !== null && p90Top !== null && <><span className="trend-range" style={{ top: `${p90Top}%`, height: `${Math.max(3, medianTop - p90Top)}%` }}/><i className="trend-p90" style={{ top: `${p90Top}%` }}/><i className="trend-median" style={{ top: `${medianTop}%` }}/></>}</div><div className="trend-bar-area"><span className="trend-bar" style={{ height: `${trend.days / internationalMaxDays * 100}%` }}/></div><b>{trend.year}</b></div>; })}</div><div className="trend-axis"><span>Relative P90 / median</span><span>Days above midpoint</span></div></div>
